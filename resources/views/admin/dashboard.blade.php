    ...
    <x-admin.page-header title="Dashboard" subtitle="Welcome back, {{ auth()->user()->name }}" />
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <article class="admin-card">
                ...
                <p class="admin-card-meta">Approved {{ number_format($approvedProviders) }} · Pending
                    {{ number_format($pendingProviders) }}</p>
            </article>
            <article class="admin-card">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <article class="admin-card lg:col-span-2">
                <div class="mb-4 flex items-center justify-between gap-3">
                    ...
                    <span class="text-xs uppercase tracking-[0.12em] text-zinc-500">Latest
                        {{ $recentUsers->count() }}</span>
                </div>
                        ...
                        <tbody>
                            @forelse($recentUsers as $user)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="inline-flex rounded-full px-2 py-1 text-xs font-medium {{ $user->is_blocked ? 'bg-rose-400/15 text-rose-200' : 'bg-emerald-400/15 text-emerald-200' }}">
                    ...
                    @forelse($recentOrders as $order)
                        <div class="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span
                                    class="rounded-full bg-cyan-300/15 px-2 py-1 text-[11px] uppercase tracking-[0.08em] text-cyan-100">{{ $order->status->value ?? $order->status }}</span>
                            </div>
