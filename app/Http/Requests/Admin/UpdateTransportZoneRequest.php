...
class UpdateTransportZoneRequest extends FormRequest
{
    ...
    {
        $zoneId = $this->route('transport_zone')?->id;

        return [
            'name' => ['required', 'string', 'max:120'],
            'slug' => ['required', 'string', 'max:160', Rule::unique('transport_zones', 'slug')->ignore($zoneId)],
